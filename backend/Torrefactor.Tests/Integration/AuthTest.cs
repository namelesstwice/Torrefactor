...
namespace Torrefactor.Tests.Integration
{
    ...
    public class AuthTest : BaseIntegrationTest
    {
        ...
        public async Task Should_fail_if_user_try_to_register_twice()
        {
            ...
            await client.Register("blah@blah.com", "123", "John Doe");
            var res = await client.Register("blah@blah.com", "123", "John Doe");
        ...
        public async Task Should_not_allow_to_sign_in_if_user_is_not_approved()
        {
            ...
            Assert.AreEqual(HttpStatusCode.Unauthorized, signInResponse.StatusCode);
        }

        [Test]
        public async Task Should_return_401_if_credentials_are_invalid()
        {
            var client = CreateServer().CreateAuthClient();
